...
carbon <-  readRDS("Data/nature_revision/carbon_outcomes_across_500_draws.rds")  %>% 
  rbindlist
...
megatrees <- readRDS("Data/nature_revision/MasterMegatreePerformance_with_uncertainty.rds") 
...
#Plot just carbon and with uncertainty
#_______________________________________
library(ggplot2)
library(dplyr)
...
# Plot
plot <- carbon_df %>%
  ggplot(aes(x = production_target, y = TOTcarbon_ACD_mean)) +
  
  # Uncertainty ribbon (95% CI)
  # Error bars (95% CI)
  geom_errorbar(aes(
    ymin = TOTcarbon_ACD_lwr95,
...
#_______________________________________
#Plot just megatree and with uncertainty
#_______________________________________
library(ggplot2)
library(dplyr)

# Filter to just AllPrimary starting scenarios
megatree_df <- megatrees %>%
  filter(scenarioStart == "all_primary")

# Plot


# Define a jitter position
jit <- position_jitter(width = 0.0, height = 0)

plot <- ggplot(megatree_df, aes(x = production_target, y = landscape_prop)) +
  
  # Error bars (95% CI)
  geom_errorbar(aes(
    ymin = landscape_prop_lwr,
    ymax = landscape_prop_upr,
    colour = case_when(
      scenarioStart %in% c("all_primary", "primary_deforested") ~ hexP,
      scenarioStart %in% c("mostly_1L", "mostly_1L_deforested") ~ hex1L,
      scenarioStart %in% c("mostly_2L", "mostly_2L_deforested") ~ hex2L
...
years",
  ...
  C <- carbon %>% 
    ...
    filter(scenarioName %in% {{scenarioFilter}}) %>% 
    master_plot_fun(y_ggplot =  TOTcarbon_ACD_mean/1000000000, #TOTcarbon_ACD_impact/1000000000,
                    y_geom_point = TOTcarbon_ACD_mean/1000000000,
